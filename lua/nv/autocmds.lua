...
autocmd("BufReadPre", {
  ...
  callback = function(args)
    ...
    vim.b[args.buf].large_buf = (ok and stats and stats.size > vim.g.max_file.size) or
        vim.api.nvim_buf_line_count(args.buf) > vim.g.max_file.lines
  end,
})
...
autocmd({ "BufAdd", "BufEnter", "TabNewEntered" }, {
  ...
  callback = function(args)
    local buf_utils = require("nv.utils.buffer")
...
autocmd({ "BufDelete", "TermClose" }, {
  ...
  callback = function(args)
    ...
    vim.t.bufs = vim.tbl_filter(require("nv.utils.buffer").is_valid, vim.t.bufs)
    if removed then nvevent("BufsUpdated") end
...
autocmd("ColorScheme", {
  ...
  group = augroup("nvim_highlights", { clear = true }),
  callback = function()
    if vim.g.colors_name then
      for _, module in ipairs({ "init", vim.g.colors_name }) do
        for group, spec in pairs(nv.user_opts("highlights." .. module)) do
          vim.api.nvim_set_hl(0, group, spec)
        end
      end
    end
    nvevent("ColorScheme", false)
  end,
})
...
autocmd({ "BufReadPost", "BufNewFile", "BufWritePost" }, {
  ...
  callback = function(args)
    ...
    if not (current_file == "" or vim.api.nvim_get_option_value("buftype", { buf = args.buf }) == "nofile") then
      nvevent("File")
      if require("nv.utils.git").file_worktree() or utils.cmd({ "git", "-C", vim.fn.fnamemodify(current_file, ":p:h"), "rev-parse" }, false) then
        nvevent("GitFile")
