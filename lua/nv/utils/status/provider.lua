...
--
-- This module can be loaded with `local provider = require "nv.utils.status.provider"`
--
-- @module nv.utils.status.provider
-- @copyright 2023
-- @license GNU General Public License v3.0

local M = {}

local condition = require("nv.utils.status.condition")
local env = require("nv.utils.status.env")
local status_utils = require("nv.utils.status.utils")
...
-- @usage local heirline_component = { provider = require("nv.utils.status").provider.fill }
...
-- @usage local heirline_component = { provider = require("nv.utils.status").provider.signcolumn }
-- @see nv.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("nv.utils.status").provider.numbercolumn }
-- @see nv.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("nv.utils.status").provider.foldcolumn }
-- @see nv.utils.status.utils.stylize
function M.foldcolumn(opts)
  opts = extend_tbl({ escape = false }, opts)
  local ffi = require("nv.utils.ffi") -- get C extensions
...
-- @usage local heirline_component = { provider = require("nv.utils.status").provider.tabnr() }
function M.tabnr()
...
-- @usage local heirline_component = { provider = require("nv.utils.status").provider.spell() }
-- @see nv.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("nv.utils.status").provider.paste() }
-- @see nv.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("nv.utils.status").provider.macro_recording() }
-- @see nv.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("nv.utils.status").provider.showcmd() }
-- @see nv.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("nv.utils.status").provider.search_count() }
-- @see nv.utils.status.utils.stylize
function M.search_count(opts)
  local search_func = vim.tbl_isempty(opts or {}) and function() return vim.fn.searchcount() end or
...
-- @usage local heirline_component = { provider = require("nv.utils.status").provider.mode_text() }
-- @see nv.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("nv.utils.status").provider.percentage() }
-- @see nv.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("nv.utils.status").provider.ruler({ pad_ruler = { line = 3, char = 2 } }) }
-- @see nv.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("nv.utils.status").provider.scrollbar() }
-- @see nv.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("nv.utils.status").provider.close_button() }
-- @see nv.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("nv.utils.status").provider.filetype() }
-- @see nv.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("nv.utils.status").provider.filename() }
-- @see nv.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("nv.utils.status").provider.file_encoding() }
-- @see nv.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("nv.utils.status").provider.file_format() }
-- @see nv.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("nv.utils.status").provider.unique_path() }
-- @see nv.utils.status.utils.stylize
function M.unique_path(opts)
  ...
  return function(self)
    ...
    return status_utils.stylize(
      (opts.max_length > 0 and #unique_path > opts.max_length and string.sub(unique_path, 1, opts.max_length - 2) .. get_icon("Ellipsis") .. "/")
      or unique_path,
      opts
    )
  end
...
-- @usage local heirline_component = { provider = require("nv.utils.status").provider.file_modified() }
-- @see nv.utils.status.utils.stylize
function M.file_modified(opts)
  opts = extend_tbl({ str = "", icon = { kind = "FileModified" }, show_empty = true }, opts)
  return function(self) return status_utils.stylize(condition.file_modified((self or {}).bufnr) and opts.str or nil, opts) end
end
...
-- @usage local heirline_component = { provider = require("nv.utils.status").provider.file_read_only() }
-- @see nv.utils.status.utils.stylize
function M.file_read_only(opts)
  opts = extend_tbl({ str = "", icon = { kind = "FileReadOnly" }, show_empty = true }, opts)
  return function(self) return status_utils.stylize(condition.file_read_only((self or {}).bufnr) and opts.str or nil,
      opts) end
end
...
-- @usage local heirline_component = { provider = require("nv.utils.status").provider.file_icon() }
-- @see nv.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("nv.utils.status").provider.git_branch() }
-- @see nv.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("nv.utils.status").provider.git_diff({ type = "added" }) }
-- @see nv.utils.status.utils.stylize
function M.git_diff(opts)
  ...
  return function(self)
    ...
    return status_utils.stylize(
    status and status[opts.type] and status[opts.type] > 0 and tostring(status[opts.type]) or "", opts)
  end
end
...
-- @usage local heirline_component = { provider = require("nv.utils.status").provider.diagnostics({ severity = "ERROR" }) }
-- @see nv.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("nv.utils.status").provider.lsp_progress() }
-- @see nv.utils.status.utils.stylize
function M.lsp_progress(opts)
  local spinner = utils.get_spinner("LSPLoading", 1) or { "" }
  return function()
    local _, Lsp = next(nv.lsp.progress)
...
-- @usage local heirline_component = { provider = require("nv.utils.status").provider.lsp_client_names({ expand_null_ls = true, truncate = 0.25 }) }
-- @see nv.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("nv.utils.status").provider.treesitter_status() }
-- @see nv.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("nv.utils.status").provider.str({ str = "Hello" }) }
-- @see nv.utils.status.utils.stylize
function M.str(opts)
