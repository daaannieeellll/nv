...
--
-- This module can be loaded with `local hl = require "nv.utils.status.hl"`
--
-- @module nv.utils.status.hl
-- @copyright 2023
...
-- @usage local heirline_component = { provider = "Example Provider", hl = require("nv.utils.status").hl.mode },
...
-- @usage local heirline_component = require("heirline.utils").surround({ "|", "|" }, require("nv.utils.status").hl.mode_bg, heirline_component),
...
-- @usage local heirline_component = { provider = require("nv.utils.status").provider.fileicon(), hl = require("nv.utils.status").hl.filetype_color },
function M.filetype_color(self)
  local devicons_avail, devicons = pcall(require, "nvim-web-devicons")
  if not devicons_avail then return {} end
  local _, color = devicons.get_icon_color(vim.fn.fnamemodify(vim.api.nvim_buf_get_name(self and self.bufnr or 0), ":t"),
    nil, { default = true })
  return { fg = color }
end
...
-- @usage local heirline_component = { provider = "Example Provider", hl = require("nv.utils.status").hl.get_attributes("treesitter") },
...
-- @usage local heirline_component = { provider = "Example Provider", hl = require("nv.utils.status").hl.file_icon("winbar") },
function M.file_icon(name)
  local hl_enabled = env.icon_highlights.file_icon[name]
  return function(self)
    if hl_enabled == true or (type(hl_enabled) == "function" and hl_enabled(self)) then return M.filetype_color(self) end
  end
end
