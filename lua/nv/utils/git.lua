--- ### Git LUA API
--
-- This module can be loaded with `local git = require "nv.utils.git"`
--
-- @module nv.utils.git
-- @copyright 2022
...
--- Run a git command from the installation directory
---@param args string|string[] the git arguments
---@return string|nil # The result of the command or nil if unsuccessful
function git.cmd(args, ...)
  if type(args) == "string" then args = { args } end
  return require("nv.utils").cmd(vim.list_extend({ "git", "-C", nv.install.home }, args), ...)
end
...
--- Check if the installation directory is a git repo
...
---@return string|nil # The branch of the installation
...
function git.remote_head(remote, branch, ...) return trim_or_nil(git.cmd(
  { "rev-list", "-n", "1", remote .. "/" .. branch }, ...)) end
...
function git.parse_remote_url(str) return vim.fn.match(str, require("nv.utils").url_matcher) == -1 and
  git.url .. str .. ".git" or str end
...
---@param commits string[] an array like table of commit messages
---@return string[][] # An array like table of echo messages to provide to nvim_echo or nv.echo
