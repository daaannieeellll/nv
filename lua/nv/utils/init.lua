...
--
-- This module can be loaded with `local utils = require "nv.utils"`
--
-- @module nv.utils
-- @copyright 2022
...
---@param kind string The kind of icon in nv.icons to retrieve
...
function M.get_icon(kind, padding, no_fallback)
  ...
  if not M[icon_pack] then
    M.icons = require("nv.icons.nerd_font")
    M.text_icons = require("nv.icons.text")
  end
  local icon = M[icon_pack] and M[icon_pack][kind]
  return icon and icon .. string.rep(" ", padding or 0) or ""
end

--- Get a icon spinner table if it is available. Icons in format `kind1`,`kind2`, `kind3`, ...
...
---@param event string The event name to be appended to NV
---@param delay? boolean Whether or not to delay the event asynchronously (Default: true)
function M.event(event, delay)
  local emit_event = function() vim.api.nvim_exec_autocmds("User", { pattern = "NV" .. event, modeline = false }) end
  if delay == false then
    emit_event()
  else
