...
--
-- This module can be loaded with `local buffer_utils = require "nv.utils.buffer"`
--
-- @module nv.utils.buffer
-- @copyright 2022
...
---@param compare_func string|function a string of a comparator defined in require("nv.utils.buffer").comparator or a custom comparator function
---@param skip_autocmd boolean|nil whether or not to skip triggering NVBufsUpdated autocmd event
...
local function unique_path(bufnr)
  return require("nv.utils.status.provider").unique_path()({ bufnr = bufnr }) .. fnamemodify(bufinfo(bufnr).name, ":t")
end
